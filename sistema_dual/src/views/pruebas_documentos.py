    ...
    with tab1:
        ...
        if st.button("Generar Prueba Anexo 5.1", use_container_width=True):
            with st.spinner("Compilando documento con datos del Universo MOCK..."):
                ...
                pdf_path = os.path.join(out_dir, f"Prueba_Anexo_5.1_MOCK.pdf")
                ...
                if success:
                    active_path = pdf_path if (to_pdf_5_1 and os.path.exists(pdf_path)) else pdf_path.replace('.pdf', '.docx')
                    auto_download_file(active_path, os.path.basename(active_path))
    ...
    with tab2:
        ...
        if st.button("Generar Prueba Anexo 5.4", use_container_width=True):
            with st.spinner("Compilando documento e insertando gráficas simuladas..."):
                ...
                import tempfile
                out_dir = tempfile.gettempdir()
                pdf_path = os.path.join(out_dir, f"Prueba_Anexo_5.4_MOCK.pdf")
                ...
                if success:
                    active_path = pdf_path if (to_pdf_5_4 and os.path.exists(pdf_path)) else pdf_path.replace('.pdf', '.docx')
                    auto_download_file(active_path, os.path.basename(active_path))
                    st.success("¡Documento generado! La descarga comenzará automáticamente.")
        ...
        with tab_obj:
            ...
            if st.button(f"Generar {out_prefix.replace('_', ' ')}", use_container_width=True, key=gen_btn_key):
                with st.spinner("Generando documento con Universo MOCK..."):
                    ...
                    import tempfile
                    out_dir = tempfile.gettempdir()
                    pdf_path = os.path.join(out_dir, f"Prueba_{out_prefix}_MOCK.pdf")
                    ...
                    if success:
                        active_path = pdf_path if (to_pdf and os.path.exists(pdf_path)) else pdf_path.replace('.pdf', '.docx')
                        auto_download_file(active_path, os.path.basename(active_path))
                        st.success("¡Documento generado! La descarga comenzará automáticamente.")
