...
def get_anexo_5_1_data(student_id):
    ...
    # Asumimos que hay un proyecto activo. Si no, tomamos el último.
    res_proj = supabase.table("proyectos_dual").select("*, unidades_economicas(*), mentores_ue(*)").eq("alumno_id", student_id).order("created_at", desc=True).limit(1).execute()
    
    if not res_proj.data:
        return None, "El alumno no tiene un proyecto dual registrado."
    
    proyecto = res_proj.data[0]
    ue = proyecto.get("unidades_economicas", {})
    # mentor = proyecto.get("mentores_ue", {}) # No se usa directamente en las etiquetas estándar pero útil saberlo
    ...
    context = {
        ...
        "dia_visita": "Por definir", # No está en BD
