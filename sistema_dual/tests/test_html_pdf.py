...
from src.utils.pdf_generator_html import generate_pdf_from_html
...
def test_html_pdf():
    ...
    template_path = os.path.join(os.path.dirname(__file__), '../src/templates/docs/Anexo_5.1_Plan_de_Formacion.html')
    if not os.path.exists(template_path):
        print(f"⚠️ Plantilla no encontrada: {template_path}")
        return

    success, msg = generate_pdf_from_html("ignored", context, output_path, template_path=template_path)
